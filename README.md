...
The **Tournament** section (`/tournament`) is available to signed-in users. Admins and superadmins can create leagues, choose arbitrary start/end dates, enter registered Karton players directly in event results, and run Swiss events. All signed-in users can browse leagues and published results. Members also see their league rank, points, attendance, and event history.
...
Admins fill ordered placement rows from #1 to #N using registered-player autocomplete. Selected players cannot be selected twice, and removing a row renumbers the remaining places. League participation is inferred from published results; there is no separate league registration step. At least two participants are required to publish. The preview and server use the same scoring function, and the server independently validates all input.
...
Deleting a Karton account preserves its league membership record and historical scores with a retained name. Archiving a league pauses event editing and preserves its standings; admins can reopen it in settings. Date edits must still include all existing events. Leagues are independent: there is no annual reset or automatic carryover.

### Swiss rounds

On an empty event, enter registered Karton players and optional commanders first. Then choose **Enter results** to order those players by final placement, or **Run Swiss rounds** to choose the round count and generate round 1. Switching modes before saving retains the same players and commanders. Supported fields are 2–128 players and 1–16 rounds, capped at players minus one. Registration is fixed after starting; this version has no drops, late registration, playoffs.

Click each player’s score button to cycle 0 → 1 → 2 → 0. A score of 2 highlights the winning player. If the other player was already at 2, their score becomes 1 so the match stays valid. Reaching two game wins automatically determines the winner. Lower scores stay in progress while saving. Advancing to the next round or reviewing final standings finalizes entered scores: the higher score wins and 1–1 draws. Untouched 0–0 matches still block advancement. Invalid scores such as 2–2 are rejected. Game scores persist with results and revision history; older outcome-only results remain intact until explicitly replaced. Matches award 3 points for a win, 1 for a draw, and 0 for a loss. An odd field receives one bye worth 3 points, preferring players with fewer byes and lower standings. Round 1 uses a persisted random seed; subsequent pairings prefer similar scores and search for a complete pairing without repeated opponents. If none exists, repeat opponents are explicitly marked. Pairing search is bounded and returns an actionable error rather than blocking the server indefinitely.

Swiss standings use match points, opponents’ total match points (Buchholz, excluding byes), match wins, then the initial random seed. This is Karton's match-level rule set, not a certified implementation of an external tournament policy. Swiss points are separate from league points. Saving match results leaves the pairing timestamp unchanged; generating the next round closes player score entry for previous matches and starts a new persisted elapsed timer. Signed-in viewers can see live pairings, match results, standings, and a timer; their page refreshes every 15 seconds while visible. Score forms retain unsaved edits during background refreshes and reject stale submissions. Either player can save the BO3 score for their own current pairing. Other tables are read-only, and byes cannot be edited. Match revisions prevent an opponent or organizer’s newer correction from being overwritten, while different tables can submit independently. Organizers alone advance rounds and publish final results. Each submission records its author in tournament history.

Organizers can select any completed round and correct its scores. Later opponents, scores, byes and pairing timestamps are preserved. If a match outcome changes, already-generated later rounds are flagged as potentially unbalanced; future pairings use the corrected standings. Corrections must leave every match complete, are revision-checked and audited, and are blocked for archived leagues. Correcting a finished tournament recalculates final placements (replacing manual adjustments) and, if published, league awards atomically; published corrections require a reason.

After the last round, **Review final standings** prepares ordered draft placements with the existing league scoring formula. Review or adjust those placements, then publish. Swiss state is included in every revision snapshot. Published league awards still change only through the existing audited results flow.

Organizers can delete a tournament from its page using the trash button and confirmation. Deletion removes the event from lists, personal history, and league awards; direct links return 404 and score entry closes. Audit history and results are retained internally with a deletion timestamp and actor. Archived leagues must be reopened before deleting events.

### Migration and verification

`migrations/0007_tournaments.sql` adds five tables for leagues, members, events, results, and revision history. `migrations/0008_tournament_commanders.sql` adds the optional commander field to event results. `migrations/0009_tournament_swiss.sql` adds persisted Swiss registration, rounds, results, and pairing timestamps to events. `migrations/0010_tournament_deletion.sql` adds the deletion timestamp. Run `npm run db:migrate` with `DATABASE_URL_ADMIN` before starting the updated app. The Docker entrypoint already runs migrations on startup.
